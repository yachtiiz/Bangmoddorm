
<?php include 'header.php' ?>

                <form class="form-horizontal" />
                <fieldset>
                    <legend><span>Owner</span> System</legend>
                    <div class="span4">
                        <a href="addDormitory.jsp"><button type="button" class="btn btn-primary btn-lg btn-block book-now2">Add Dormitory</button></a><br><br>
                        <a href="checkBooking.jsp"><button type="button" class="btn btn-primary btn-lg btn-block book-now2">Check Booking History</button></a>
                    </div>
                    <br />
                    <br />
                    <div style="margin-top: 70px">
                    <legend><span>Edit Your</span> Domitory</legend>
                    <div class="span4">
                        <a href="editDormitory.jsp"><button type="button" class="btn btn-primary btn-lg btn-block book-now2">My Place</button></a><br><br>
                        <a href="editDormitory.jsp"><button type="button" class="btn btn-primary btn-lg btn-block book-now2">My Place 2</button></a><br>
                    </div>
                    <div class="span4">
                        <a href="editRoom.jsp"><button type="button" class="btn btn-primary btn-lg btn-block book-now2">Add Room</button><br><br></a>
                        <a href="editRoom.jsp"><button type="button" class="btn btn-default btn-lg btn-block book-now2">Add Room</button></a>
                    </div>
                    </div>
                </fieldset>
                </form>
</div> <!-- /container -->
<br><br><br><br><br><br><br>
<?php include 'footer.php' ?>
